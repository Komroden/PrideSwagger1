    ...
    return (
        <>
            ...
            <div className="main_for_all_pages">
                ...
                {(filter==='all'||filter==='active')&&contests.active.map((item,index)=>
                    <DrawItem id={item.id} key={item.id} imgPrice={item.image} priceAdd={item.participationCost} title={item.caption} desc={item.desc} date={item.finishDate}
                    members={item.participantsCount} startDate={item.startDate}/>
                    )}

                <div className="rozgr_archive">
                    {(filter==='all'||filter==='past')&&contests.past.map((item,index)=>
