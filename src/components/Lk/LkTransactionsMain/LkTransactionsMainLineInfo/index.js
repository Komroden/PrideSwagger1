...
export const LkTransactionsMainLineInfo = ({status,date,firstName,lastName,email,id}) => {
    ...
    return (
        <div  className="lineinfo">
            ...
            <Collapse  in={open}  unmountOnExit>
                ...
                {value.map(item=><Sub key={item.id} status={item.isActivated} date={item.creationDate} firstName={item.firstName} lastName={item.lastName} email={item.email} id={item.id} />)}
                </div>
            </Collapse>
        </div>
    );
